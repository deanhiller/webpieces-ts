 ...
 * the logged context fields — read DIRECTLY from the active
 * {@link RequestContext} (secured values masked) — into the bunyan fields object,
 * and normalizes an optional Error into `err: { name, message, stack }` — matching
 * the tested trytami AppLogger. The GCP stream then serializes those fields into
