...
- Turning API decorators into invokable GET/POST routes (`ApiRoutingFactory`, `RouteHandler`, `MethodMeta`) with the same explicit path/query/body argument metadata used by generated HTTP clients — including SKIPPING an `@WpAuthLocalOnly` route entirely when `RuntimeLocality` says this process is not a developer machine, so the endpoint does not exist off-local
- Enforcing every auth mode at request time (`AuthFilter`: `@WpAuthPublic`, `@WpAuthJwt`, `@WpAuthOidc`, `@WpAuthSharedSecret`, `@WpAuthWebhook`, `@WpAuthApiKey`, `@WpAuthLocalOnly`), and declaring the process's locality at startup from `RuntimeSetupOptions.locality`
