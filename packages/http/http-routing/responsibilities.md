...
- Turning API decorators into invokable routes (`ApiRoutingFactory`, `RouteHandler`, `MethodMeta`) — including SKIPPING an `@WpAuthLocalOnly` route entirely when `RuntimeLocality` says this process is not a developer machine, so the endpoint does not exist off-local
