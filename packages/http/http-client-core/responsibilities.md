...
- Translating HTTP responses into the typed `ApiError` taxonomy (`ClientErrorTranslator`), returning a `TranslatedFailure` that also records WHO decided — an app-registered `ClientRegistry` translation, or the built-in default mapping. A Webpieces body is decoded by semantic `kind` and its HTTP status is validated; status-only reconstruction is the fallback for foreign responders. `ApiEndUserError.message` and `ApiBadRequestError.callerMessage` are the caller-safe prose fields.
