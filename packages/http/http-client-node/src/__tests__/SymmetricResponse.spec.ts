...
import { DnsAddressResolver } from '../AddressResolver';
...
describe('node client whole-response symmetry', () => {
    it('reconstructs the server error from status, reason, repeated headers and body', async () => {
        ...
        proxy.init(TestApi, new ClientConfig('symmetry-test'), []);
        const client = buildClientProxy(TestApi, proxy);
