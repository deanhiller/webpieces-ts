...
- Owning a downstream 4xx as THIS server's 500 (`NodeProxyClient.adaptDownstreamFailure`). A status from a dependency describes OUR request to it — a wrong path, a wrong base URL, an undeployed dependency, bad service credentials — so relaying it would let an internal misconfiguration impersonate a legitimate answer to an external consumer. The downstream diagnostic survives as the 500's `httpCause`. 5xx / 598 pass through; the opt-out is an app-registered `ClientRegistry` error translation, not a config flag
