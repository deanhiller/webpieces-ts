...
- The six auth-mode decorators on an api contract — `@Public`, `@AuthJwt`, `@AuthOidc`, `@AuthSharedSecret`, `@AuthWebhook`, `@AuthLocalOnly` — and the `AuthMode` discriminated union every reader switches on.
