...
- `RouteMetadataFactory.assertNoDuplicateRoutes` — refusing a contract in which two endpoints resolve to the same HTTP method + path. Empty paths are legal (`@ApiPath('')` + `@Endpoint('')` joins to `''`, so a base URL is used byte for byte); ambiguity is what is rejected
