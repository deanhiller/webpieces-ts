...
- MCP/API DTO schema metadata (`@WpDto`, `@WpDtoField` with `WpDtoFieldOptions` or `WpDtoMapFieldOptions` for typed `Record` maps, `DtoSchemaBuilder`) and `DtoSchemaBuilder.isClosedSchema`, the one closure gate that accepts both `additionalProperties: false` and a typed-map value schema.
