...
export class WebpiecesCoreHeaders {
     ...
     *
     * - `httpHeader` `x-request-id` → travels DOWN the call tree on every outbound request.
     * - `responseHeader` `x-request-id` → and back UP on every response, which is the header a
     *   support desk quotes. This used to be a hard-coded `res.setHeader` in `ExpressWrapper`
     *   (`stampTransactionId`), reachable by no other key. It is an ordinary registry entry now: the
     *   server's ONE response choke point writes every key in
     *   {@link HeaderRegistry.getResponseTxfrKeys}, so a second key joins with no framework edit.
     * - `responseMerge` `first` → and NOT `collect`. A callee echoes OUR id back at us, so the
     *   caller's context must KEEP what it already has. `collect` would turn `requestId` into a
     *   growing array on every client call and break every log line of the request; `last` would
     *   rewrite it with an echo. `first` makes reading it back a provable no-op for the normal case
     *   and still adopts an id when a caller somehow had none.
     */
    static readonly REQUEST_ID = ContextKey.untrusted<string>(
        'requestId',
        'x-request-id',
        /*maskInLogs*/ false,
        /*isLogged*/ true,
