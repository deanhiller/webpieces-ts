...
const GRAPH: EnhancedGraph = {
    'angular-site': { level: 3, dependsOn: ['http-client'], framework: 'angular' },
    server2: { level: 4, dependsOn: ['http-client'], framework: 'express' },
    'http-client': { level: 2, dependsOn: [], framework: 'all' },
};

describe('generateDot', () => {
    it('colors each node by its framework', () => {
    ...
    it('keeps the level and adds the framework in each label', () => {
        const dot = generateDot(GRAPH);
        expect(dot).toContain('label="angular-site\\n(L3 · angular)"');
        expect(dot).toContain('label="http-client\\n(L2 · all)"');
    });

    it('treats an absent framework as "all" (grey)', () => {
    ...
    it('uses the default color for an unknown framework value', () => {
        const dot = generateDot({ odd: { level: 0, dependsOn: [], framework: 'vue' } });
...
describe('generateHTML', () => {
    it('renders a framework legend', () => {
        const html = generateHTML(generateDot(GRAPH));
        expect(html).toContain('colored by framework');
