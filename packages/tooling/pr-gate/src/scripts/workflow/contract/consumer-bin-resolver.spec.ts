...
describe('ConsumerBinResolver', () => {
    it("resolves the CONSUMER's install by walking up from the project, the way node does", () => {
        ...
        FakeInstall.at(workspace, '0.4.810');
        ...
        expect(bin.version).toBe('0.4.810');
    ...
    it('NEVER takes a copy that is not under the consumer — a bundled copy beside the plugin is invisible', () => {
        ...
        expect(message).toContain('Fix Option 1: (preferred) Add it as a devDependency: @webpieces/openapi-generator >= 0.4.807');
    });
    ...
    it('fails the version handshake with the bump as its cure', () => {
        ...
        FakeInstall.at(consumer, '0.4.806');

        const message = refusal([consumer]);

        expect(message).toContain('@webpieces/openapi-generator 0.4.806 is installed');
        expect(message).toContain('Fix Option 1: (preferred) Bump the devDependency to @webpieces/openapi-generator >= 0.4.807');
    });
    ...
    it('refuses a workspace link (0.0.0-dev): a prerelease is not a release', () => {
        ...
        expect(message).toContain('@webpieces/openapi-generator >= 0.4.807');
    });

    it('accepts a release equal to the minimum, and any newer minor or major', () => {
        for (const version of ['0.4.807', '0.5.0', '1.0.0']) {
            const consumer = specTempDirs.make('wp-consumer-bin-');
