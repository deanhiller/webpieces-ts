...
// long before any build has produced `src/scripts/wp-push-dev.js`. Pointing `bin` straight at the
...
const compiled = path.join(__dirname, '..', 'src', 'scripts', 'wp-push-dev.js');

if (!fs.existsSync(compiled)) {
    console.error(
        '  [pr-gate] wp-push-dev: package not built yet. Run the build first, or install from npm.',
    );
    process.exit(1);
}
