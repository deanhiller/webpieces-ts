...
- An app that deliberately wants to publish richer text sets an `ErrorTranslators` object with
  `ClientRegistry.setErrorTranslators`; its `toWire()` result is sent verbatim. That is the explicit, greppable opt-out.
