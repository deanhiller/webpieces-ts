...
import {
    ...
    ADD_HOOK_PKG_CMD, CHECKOUT_MAIN_PULL_CMD, HOOK_PKG, WORKSPACE_MANIFEST, PACKAGE_MANIFEST,
} from './l0-allowlist';
...
const DENY_REASON_SH = `if [ -n "\$BROKEN_BIN" ]; then
  ...
  if [ -z "\$WP_HOOK_PKG_DECLARED" ]; then
    ...
    REASON="\$WP_HEAD\${NL}\${NL}${l0GuardHeader(L0_FAULT_UNDECLARED, '1 violation')}\${NL}  ${HOOK_PKG} is NOT declared in this tree's root package.json, and is not installed (\${BIN_NAME} not found)\${NL}    → .claude/settings.json still runs its hooks, so every OTHER tool call is BLOCKED. A BARE 'pnpm install' is a NO-OP here: the root manifest asks for nothing, so it reports 'Lockfile is up to date' and converges to this same state. CHANGE something first - sync, or the pin - and install AFTER that.\${NL}    → ${l0MatrixCitation(L0_FAULT_UNDECLARED)}\${NL}\${NL}\${WP_STILL_ALLOWED}\${NL}\${NL}  Fix Option 1: (preferred) ${HOOK_PKG} normally arrives WITH @webpieces/nx-webpieces-rules, the umbrella that bundles the whole toolchain - so the usual cause is a tree that is behind, not a missing declaration. Sync, then install:\${NL}    run EXACTLY: '${CHECKOUT_MAIN_PULL_CMD}'\${NL}    then run EXACTLY: 'pnpm install'\${NL}  Fix Option 2: still missing? the pinned release predates the umbrella carrying it. Raise the ONE catalog entry in ${WORKSPACE_MANIFEST} to a newer @webpieces/nx-webpieces-rules - editing that file is allowed while this block is up - then:\${NL}    run EXACTLY: 'pnpm install'\${NL}  Fix Option 3: LAST RESORT, only to unblock yourself right now\${NL}    run EXACTLY: '\$WP_ADD_CMD'\${NL}    then REVERT IT BEFORE YOU COMMIT: 'pnpm remove ${HOOK_PKG}'. A direct root dependency on ${HOOK_PKG} violates the umbrella rule - the root manifest depends on @webpieces/nx-webpieces-rules ALONE - so it must never reach a PR. It unblocks this session; it is not the fix.\${NL}  NOT an option: if you removed ${HOOK_PKG} on purpose, delete its hooks from .claude/settings.json instead.\${NL}\${NL}${NO_CHAINING_RULE}"
  else
    # The HEADLINE, kept in its own variable so DENY_EMIT_SH can paint ONLY it red (see there).
