...
import { NEIGHBOUR_SURFACE_SUFFIX } from './neighbour-hooks';
import { REVIEWER_AGENT_MARKER } from './reviewer-agent';
...
class ShimStaleDeny {
    ...
    private header(): string[] {
        ...
        return [
            ...
            `    → webpieces manages those files as ONE set, GENERATED by the INSTALLED @webpieces/ai-hook-rules${verNote}; what is on disk is missing, reverted, hand-edited, or predating this binary. The env entry is ${BASH_CWD_ENV_KEY}=${BASH_CWD_ENV_VALUE}, which pins the Bash cwd to the project root, identically for every subagent because settings env is inherited.`,
            ...this.neighbourNote(),
            ...this.reviewerAgentNote(),
    ...
    /**
     * The fourth surface's line, spent ONLY when that surface is the one that moved.
     *
     * Conditional rather than always-on because of what it is: the other three are webpieces' own files
     * and a reader meeting fault S needs no introduction to them, whereas this one is about the READER'S
     * hooks, registered beside ours, and the way it fails is the one thing they cannot infer — a relative
     * entry path that fails to load exits non-zero, which the hooks reference defines as NON-BLOCKING, so
     * the guard silently stops guarding rather than blocking. Printing that when nothing relative is
     * registered would be argument inside a message with a hard budget; printing it when the surface IS
     * drifted is the only place the reader learns why a guard they wrote was not running.
     */
    private neighbourNote(): string[] {
        if (!this.drifted.some((s: string): boolean => s.endsWith(NEIGHBOUR_SURFACE_SUFFIX))) return [];
        return ['    → the neighbour-hooks surface is YOUR hooks beside ours: a relative entry path fails to load off the repo root, and that exit is NON-BLOCKING, so the guard silently stops guarding. Fix Option 1 anchors each one and prints it.'];
    }

    /**
     * The reviewer agent's line, spent ONLY when it is the surface that moved — it is the one managed file
     * that is not part of the hook itself, so a reader cannot infer why a PR-review agent blocks a Bash call.
     */
