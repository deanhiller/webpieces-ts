...
class ShimStaleDeny {
    ...
    private header(): string[] {
        ...
        return [
            ...
            `    → webpieces manages those FOUR things as ONE set, GENERATED by the INSTALLED @webpieces/ai-hook-rules${verNote}; what is on disk is reverted, hand-edited, or predating this binary. The env entry is ${BASH_CWD_ENV_KEY}=${BASH_CWD_ENV_VALUE}, which pins the Bash cwd to the project root, identically for every subagent because settings env is inherited.`,
            ...this.neighbourNote(),
    ...
    private neighbourNote(): string[] {
        ...
        return ['    → the fourth is YOUR hooks beside ours: a relative entry path fails to load off the repo root, and that exit is NON-BLOCKING, so the guard silently stops guarding. Fix Option 1 anchors each one and prints it.'];
    }
