...
import { AGENTS_STATE_DIR } from '@webpieces/rules-config';
...
function readLog(root: string, name: string, agentId: string = ''): string {
    const dir = agentId === ''
        ? path.join(root, '.webpieces', 'logs')
        : path.join(root, '.webpieces', 'logs', AGENTS_STATE_DIR, agentId);
...
/**
 * PER-AGENT SEPARATION, through the writers themselves. state-dir-agent-logs.spec.ts owns the path
 * algebra; this owns the wiring — that the identity captured at begin() is the one that decides where
 * finish() writes, several frames later, at a terminal boundary that has no payload in reach.
 */
describe('concurrent agents do not interleave into one log', () => {
    it('writes the coordinator to today\'s path, byte-for-byte unchanged', () => {
        const root = tmpRoot();
        const coordinator = new AgentIdentity('', '');
        expect(coordinator.coordinator).toBe(true);

        const invocations = new InvocationLog();
        invocations.begin(root, 'Bash', 'ls', coordinator);
        invocations.finish('ALLOW', '-');

        expect(fs.existsSync(path.join(root, '.webpieces', 'logs', INVOCATION_LOG))).toBe(true);
        expect(fs.existsSync(path.join(root, '.webpieces', 'logs', AGENTS_STATE_DIR))).toBe(false);
        expect(readLog(root, INVOCATION_LOG)).toContain('\tagent=coordinator');
    });

    it('sends two different agentIds to two different files', () => {
        const root = tmpRoot();
        for (const id of ['agent-one', 'agent-two']) {
            const invocations = new InvocationLog();
            invocations.begin(root, 'Bash', `echo ${id}`, new AgentIdentity(id, 'Explore'));
            invocations.finish('ALLOW', '-');
        }

        expect(readLog(root, INVOCATION_LOG, 'agent-one')).toContain('echo agent-one');
        expect(readLog(root, INVOCATION_LOG, 'agent-one')).not.toContain('echo agent-two');
        expect(readLog(root, INVOCATION_LOG, 'agent-two')).toContain('echo agent-two');
        // …and NOTHING landed in the coordinator's file, which is the interleaving that was happening.
        expect(fs.existsSync(path.join(root, '.webpieces', 'logs', INVOCATION_LOG))).toBe(false);
    });

    it('separates the decision log and the rejection index by agent too', () => {
        const root = tmpRoot();
        const agent = new AgentIdentity('agent-three', 'Explore');
        logGuardDecision(root, new GuardDecision('r', 'Bash', 'git push', 'dean/x', 'BLOCK', 'nope'), agent);
        const input = new NormalizedToolInput(path.join(root, 'src/x.ts'), [new NormalizedEdit('a', 'b')]);
        logRejection('Edit', input, new BlockedResult('[r] (a reason)\nno'), root, agent);

        expect(readLog(root, DECISION_LOG, 'agent-three')).toContain('git push');
        expect(readLog(root, REJECTION_LOG, 'agent-three')).toContain('agent=agent-three');
        expect(fs.existsSync(path.join(root, '.webpieces', 'logs', DECISION_LOG))).toBe(false);
    });

    // A caller that CANNOT know who it is (library consumers, the openclaw adapter) keeps today's
    // paths. Guessing it into a namespace would scatter the coordinator's own history.
    it('keeps an UNKNOWN caller on the coordinator path, while still saying it is unknown', () => {
        const root = tmpRoot();
        const invocations = new InvocationLog();
        invocations.begin(root, 'Bash', 'ls', UNKNOWN_AGENT);
        invocations.finish('ALLOW', '-');

        expect(fs.existsSync(path.join(root, '.webpieces', 'logs', INVOCATION_LOG))).toBe(true);
        expect(readLog(root, INVOCATION_LOG)).toContain('\tagent=unknown');
        expect(UNKNOWN_AGENT.coordinator).toBe(false);   // the guard verdict is unchanged by any of this
    });

    it('cannot be walked out of the logs directory by a hostile agentId', () => {
        const root = tmpRoot();
        const invocations = new InvocationLog();
        invocations.begin(root, 'Bash', 'ls', new AgentIdentity('../../../escaped', 'Explore'));
        invocations.finish('ALLOW', '-');

        const agentsDir = path.join(root, '.webpieces', 'logs', AGENTS_STATE_DIR);
        expect(fs.readdirSync(agentsDir)).toEqual(['_________escaped']);
        expect(fs.existsSync(path.join(root, '.webpieces', 'escaped'))).toBe(false);
    });
});
...
describe('a log failure never throws', () => {
    it('swallows an unwritable destination in all three writers', () => {
        ...
        const invocations = new InvocationLog();
        expect(() => {
            invocations.begin(root, 'Bash', 'ls', new AgentIdentity('a1', 'Explore'));
            invocations.finish('BLOCK', 'r', 'C');
        }).not.toThrow();

        expect(() => logGuardDecision(root, new GuardDecision('r', 'Bash', 'ls', 'b', 'BLOCK', 'why', '-', 'C'), new AgentIdentity('a1', 'Explore'))).not.toThrow();

        const input = new NormalizedToolInput(path.join(root, 'src/x.ts'), [new NormalizedEdit('a', 'b')]);
        expect(() => logRejection('Edit', input, new BlockedResult('[r] (x)\nno', 'C'), root, new AgentIdentity('a1', 'Explore'))).not.toThrow();
    });
});
