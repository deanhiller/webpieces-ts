...
export class MergedBranchBashGuardRule extends BashRuleBase<BranchStateGuardConfig> {
    ...
    readonly fixHint = new FixHint(
        ...
        [
            ...
            new Option('Still allowed here: recovery/cleanup git, read-only git status|log|diff|show|branch, gh GENERALLY (it talks to GitHub, not to this tree — but not gh repo clone / pr checkout / run download, which write local files), curl/wget without -o/-O or a > redirect, switching branches/worktrees, pnpm wp-sync-main and pnpm wp-cleanup, and installs/upgrades.'),
