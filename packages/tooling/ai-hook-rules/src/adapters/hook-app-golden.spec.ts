 ...
 *
 * `enforceCommittedShim()` compares the committed `.claude/webpieces/ai-hook.sh` against the LOCAL
 * `renderShim()`, anchored on the RUNNING MODULE's tree (governingShimRoot), which under vitest is this
 * source checkout — not the throwaway fixture repo every other input comes from. So the moment a PR
 * legitimately changes the renderer, every `guards`-mode fixture below turns into an L0 fault-S deny
 * that has nothing to do with what it tests.
 *
 * That is not a hypothetical and it is not a bug to fix by regenerating the artifact: the committed shim
 * is a GENERATED file that must stay byte-identical to the INSTALLED release's renderShim(), so it is
 * regenerated one release LATER (by `wp-upgrade-shim`, or the next upgrade) and never inside the PR that
 * changes the renderer. Regenerating it here would fire fault S for every consumer still on the old
 * release — including this repo's own sessions.
 *
 * So the probe is neutralized for the fixtures, exactly as the three process ports are, and the REAL
