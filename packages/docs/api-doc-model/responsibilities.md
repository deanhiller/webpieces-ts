...
- Per DTO: named types become model entries a renderer can `$ref`; primitives, arrays, **string enums** (`enum SpeakerGender { FEMALE = 'female' }` → a named `enum` of the member VALUES, in every position: a field, an array element, a `Record` value, a nested DTO, a request or response root) and string-literal unions → enum; ONE enum member as a type (`mode: VoiceMode.RANDOM`) → a one-value enum, and a union of members / literals → an enum of all their values (#1023). The generator keeps supporting literal unions; the `one-enum-spelling-in-api-lib` rule in `@webpieces/code-rules` is what forbids new ones in an api library, **optional vs nullable distinguished** (`{}` and `{x: null}` are different wire documents), nested objects, index signature → open map
...
- **A union of named object types becomes a union plus a DERIVED discriminator** when every branch carries the same property typed as string VALUES — one literal or string-enum member, or a UNION of them on a branch (`mode: 'a' | 'b'`, `mode: VoiceMode.A | VoiceMode.B`, #1023) — and no value is claimed by two branches. Every value maps to its branch, which OpenAPI's `mapping` allows. No invented discriminator for a union TypeScript itself cannot narrow — that is recorded as an `UnmappedType` instead
