...
**ALL catch blocks MUST use `toError(err)` as the first statement.**

This pattern is enforced by the ESLint rule `@webpieces/catch-error-pattern`.
...
### Pattern 2: Explicitly Ignored Errors

When you intentionally want to ignore an error, comment out the toError call:
...
Use numbered suffixes for nested catches:
...
| Object | Stringifies | `toError({code: 404})` → `Error("Non-Error object thrown: {...}")` |
...
The `@webpieces/catch-error-pattern` rule will flag these issues:
...
**Filter with nested error handling:**
```typescript
@injectable()
export class TransactionFilter implements Filter {
  async filter(meta: MethodMeta, next: NextFilter): Promise<Action> {
    try {
...
### Exception: Intentional Recursion Prevention

There is ONE exception where you should NOT call `toError()` - inside `toError()` itself:
...
This is the only acceptable deviation from the pattern.
...
- [ ] **Writing catch blocks?** → **Use `toError(err)` as first statement**
