        ...
        try {
            ...
            if (buttonCount > 0) {
                ...
                if (githubCount > 0) {
                    ...
                    await page.fill(
                        'input[name*="owner" i], input[placeholder*="owner" i]',
                        GITHUB_OWNER,
                    );
                    await page.fill(
                        'input[name*="repo" i], input[placeholder*="repo" i]',
                        GITHUB_REPO,
                    );
                    await page.fill(
                        'input[name*="workflow" i], input[placeholder*="workflow" i]',
                        WORKFLOW_FILE,
                    );
                    ...
                    if (submitCount > 0) {
                        console.log(
                            '   ⏸️  PAUSING - Please review the form and click Save manually',
                        );
                    ...
                    await page.screenshot({
                        path: `npm-debug-${packageName.replace('/', '-')}.png`,
                    });
                }
            } else {
                console.log(
                    '   ℹ️  Trusted publisher might already be configured or button not found',
                );
                console.log('   📸 Taking screenshot for debugging...');
