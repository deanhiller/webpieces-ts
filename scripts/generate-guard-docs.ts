...
import { renderGuaranteeRoot } from '../packages/tooling/ai-hook-rules/src/bin/guarantee-root';
 ...
 *   templates `packages/tooling/ai-hook-rules/templates/*.sh` — the POSIX-sh hooks, rendered from
 *             renderShim()/renderGuaranteeRoot(). These used to have NO command at all: their
...
function main(): void {
    ...
    wrote.push(shim);
    const guaranteeRoot = path.join(templates, 'guarantee-root.sh');
    fs.writeFileSync(guaranteeRoot, renderGuaranteeRoot(), { mode: 0o755 });
    wrote.push(guaranteeRoot);
