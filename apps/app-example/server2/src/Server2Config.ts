...
/**
 * Declare server2's filters + routes on the {@link ApiFactory}. server2 is an internal service
 * (Server2Api is @Authentication(false)), so there is NO auth filter. All filters are api-tier;
 * server2 needs no app DI modules beyond the standard company set; the controller is
