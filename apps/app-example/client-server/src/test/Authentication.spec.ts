...
/** Run a call with a single auth header published on the HttpRequest (as a transport would). */
async function withAuthHeader<T>(headerName: string, value: string, fn: () => Promise<T>): Promise<T> {
...
describe('Authentication: shared-secret (bound state)', () => {
    ...
    const overrides = new ContainerModule(async (options: ContainerModuleLoadOptions) => {
        (await options.rebind(AuthConfig)).to(TestAuthConfig); // sharedSecrets: { INTERNAL_API_SECRET: 'some-test-key' }
    });

    it('accepts the correct shared secret', async () => {
        const api = await secureClient(overrides);
        const res = await withAuthHeader('x-webpieces-shared-secret', 'some-test-key', () =>
    ...
    it('rejects a wrong shared secret (401)', async () => {
        const api = await secureClient(overrides);
        await expect(
            withAuthHeader('x-webpieces-shared-secret', 'WRONG-key', () => api.internalOp({})),
...
describe('Authentication: jwt (real signed token, role-gated)', () => {
    ...
    it('rejects a JWT missing the admin role (403)', async () => {
        ...
        await expect(
            withAuthHeader('authorization', `Bearer ${token}`, () => api.adminOp({})),
        ).rejects.toThrow(HttpForbiddenError);
    });
...
describe('Authentication: oidc (real dev token, caller = self)', () => {
    ...
    it('accepts a dev OIDC token minted for this service (self)', async () => {
        ...
        const res = await withAuthHeader('authorization', `Bearer ${token}`, () => api.serviceOp({}));
        expect(res.ok).toBe(true);
    });
