...
function clientSending(value: string): SecureApi {
    // SecureApi has an @AuthOidc endpoint, so a minter is required at construction (unused here).
    return createApiClient(
        SecureApi,
        new ClientConfig(`http://localhost:${PORT}`, undefined, mintIdToken, new Secrets({ INTERNAL_API_SECRET: value })),
    );
}
