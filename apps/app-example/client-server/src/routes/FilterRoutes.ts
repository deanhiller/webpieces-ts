...
export class FilterRoutes implements Routes {
    configure(routeBuilder: RouteBuilder): void {
        ...
        // Auth filter - enforces @Public/@Authenticated/@Roles decorators
        routeBuilder.addFilter(new FilterDefinition(1900, AuthFilter, '*'));

        // Test-case recording (activates on x-webpieces-recording header or
        // WebpiecesConfig.recordingAlwaysOn; after auth so only real flows record)
