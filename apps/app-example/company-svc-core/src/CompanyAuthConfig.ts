 ...
 * It holds ONLY the accepted @WpAuthSharedSecret values; the JWT mechanism lives on
 * {@link CompanyJwtHook} and OIDC is the framework default ({@link DefaultOidcVerifier}).
 *
 * Prod reads the expected secret VALUES from env by the @WpAuthSharedSecret name. Two slots so a
