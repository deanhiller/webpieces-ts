...
- Abstract API classes (`SaveApi`, `PublicApi`) decorated with `@ApiPath`, `@Endpoint`, and exactly one auth decorator (`@Public` / `@AuthJwt` / `@AuthOidc` / `@AuthSharedSecret` / `@AuthWebhook` / `@AuthLocalOnly`)
