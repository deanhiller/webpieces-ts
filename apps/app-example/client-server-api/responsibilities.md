...
- Abstract API classes (`SaveApi`, `PublicApi`) with `@ApiPath`, whose `@Endpoint` methods each declare exactly one method-level auth decorator (`@WpAuthPublic` / `@WpAuthJwt` / `@WpAuthOidc` / `@WpAuthSharedSecret` / `@WpAuthWebhook` / `@WpAuthLocalOnly`)
...
Contract-only library: no DI-registered classes. Decorators are read both server-side (route binding) and client-side (`ClientHttpFactory.createRpcClient`). DTO fields are optional for protocol evolution.
