...
|---|---|---|
| 1 | `.claude/webpieces/ai-hook.sh` | **absolute** via `$CLAUDE_PROJECT_DIR`, once per guard bin |
| 2 | the `.claude/settings.json` entries registering it | — |
| 3 | `.claude/settings.json` → `env.CLAUDE_BASH_MAINTAIN_PROJECT_WORKING_DIR = "1"` | — |

It was FOUR. A third hook, `.claude/webpieces/guarantee-root.sh` (L-1), is **deleted**, and so is the
relative registration it existed to protect. Both guard hooks are ABSOLUTE now, so they resolve from any
cwd and the launch guarantee is structural rather than policed — which is why `cd` into a subdirectory is
no longer denied. The relative form was adopted so each tree would run its own release; measured
2026-08-10, it never did — a linked worktree has no `node_modules`, so the shim's upward walk always ran
the MAIN tree's binary. The MAIN tree governs every tree, and when a worktree PINS a different
`@webpieces`, L1 row 8 (`trinary-version-skew`) blocks and names the fix.

Row 3 is no longer load-bearing for RESOLUTION — an absolute hook always resolves. It is kept for VERDICT
STABILITY: pinning the Bash cwd to the project root means a guard's answer depends on the command, not on
where an earlier `cd` left the shell, and because settings `env` is **inherited**, the main agent and
every subagent get the same cwd and therefore the same verdict. The trade: the cwd reset becomes silent
and unconditional, so a deliberate `cd` no longer persists between Bash calls — chain instead.

`pnpm exec wp-upgrade-shim` repairs all three and removes the retired hook, and it is the only cure that
does: the older `cp …/templates/ai-hook.sh .claude/webpieces/ai-hook.sh` fallback repairs **one of three**
and its deny text says so. Nothing validated the registration at all before it joined this fault, so a
settings file left on a superseded form silently changed who governs, with no signal anywhere.

A repair fixes the tree it is RUN IN. Because the hooks are absolute, the tree that governs the session is
the one `$CLAUDE_PROJECT_DIR` names — so run the cure there. `wp-upgrade-shim` says so when the two trees
diverge and prints the primary-tree command; `governingShimRoot()` compares the tree the running
...
partition from `L0_FAULTS` itself, so an eighth fault with no emitter fails the build.

That matters because `grep 'fault=S'` now spans the whole trail rather than only its `sh` half, and
`cut` + `uniq -c` over any one stream can be diffed against `L0_FAULTS` — which is the point of stamping
at all.
