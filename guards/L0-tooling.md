...
| 4b | `…-hook not found` / `is NOT declared in this tree's root package.json`, and `pnpm install` reports **"Lockfile is up to date"** and changes nothing | `U`; the package normally arrives WITH `@webpieces/nx-webpieces-rules`, the umbrella — so the usual cause is a tree that is merely BEHIND. Twice it was structural instead: the umbrella's edge was pruned as unused (2026-08-05), and a pinned release whose published `pr-gate` predated the dependency left the hoisted root slot empty (2026-09-03) | BLOCK | a *bare* `pnpm install` is a **no-op** here and repeating it converges to the same broken tree (2026-08-05: four identical installs, then the block was handed back to the human). CHANGE something first — sync, or raise the ONE catalog entry (`pnpm-workspace.yaml` is editable while the block is up) — and install AFTER that. `U`'s LAST cure declares the package directly: that is a **session unblock, not a fix**, because a direct root dependency violates the umbrella rule (the root manifest depends on the umbrella ALONE) — revert it before committing. 2026-09-03: it was labelled "(preferred)" with no revert, was followed exactly as written, and reached a real `package.json` |
